...
/**
 * Client-side UI state for the cash boxes (Kassalar) screen.
 * Same split as the other list modules: server data → React Query,
 * UI/interaction state (filters, paging, drawer) → this store.
 */
interface CashBoxUiState {
  page: number
  pageSize: number
  search: string
  sortBy?: string
...
export const useCashBoxStore = create<CashBoxUiState>((set) => ({
  page: DEFAULT_PAGE,
